...
[v_ref, ~, ~] = QP_with_original_feasible_set(p, o, D, D_o, D_s, v_c, k_h(controller_idx));
...
function [v_ref, A, b] = QP_with_original_feasible_set(p, o, D, D_o, D_s, vc, k_h)
...
for i_o = 1:n_o
    ...
    b(i_o, 1) = -k_h*((1/norm(p-o(:, i_o)) - 1/D_s));
end
...
if(isempty(H))
    ...
    for i = 1:size(o, 2)
        H.f1_obs_margin{i} = patch(o(1,i)+cos(theta)*(D(1)+delta_h), o(2,i)+sin(theta)*(D(1)+delta_h), 'k', 'handlevisibility', 'off', 'displayname', 'Obstacles', 'linestyle', 'none', 'facealpha', 0.1);
    ...
    H.f1_traj{1} = animatedline('Color',clr{1},'DisplayName',sprintf('$k_h = %.2f$', k_h(1)),'LineStyle',style{1}, 'LineWidth', 2);
    H.f1_traj{2} = animatedline('Color',clr{2},'DisplayName',sprintf('$k_h = %.2f$', k_h(2)),'LineStyle',style{2}, 'LineWidth', 2);
    H.f1_traj{3} = animatedline('Color',clr{3},'DisplayName',sprintf('$k_h = %.2f$', k_h(3)),'LineStyle',style{3}, 'LineWidth', 2);
    H.f1_legend  = legend('Location','northeast');
    H.f1_legend.Interpreter = 'latex';
    ...
    ctrl_last = evalin('base', 'ctrl_last');

    if(isempty(ctrl_last))
        ctrl_last = ctrl;
    end
    ...
    if(controller_idx == 1)
        ...
        addpoints(    H.f3_dist_1, t_now, min(vecnorm(p-o)) - 1);
        addpoints(  H.f3_tildev_1, t_now, norm(ctrl.v_ref   - ctrl.v));
        addpoints(H.f3_dinivref_1, t_now, norm((ctrl.v_ref-ctrl_last.v_ref) / T));
        
    elseif(controller_idx == 2)
        addpoints(H.f5_vref1_2, t_now, ctrl.v_ref(1));
        addpoints(H.f5_vref2_2, t_now, ctrl.v_ref(2));
        addpoints(    H.f3_dist_2, t_now, min(vecnorm(p-o)) - 1);
        addpoints(  H.f3_tildev_2, t_now, norm(ctrl.v_ref   - ctrl.v));
        addpoints(H.f3_dinivref_2, t_now, norm((ctrl.v_ref-ctrl_last.v_ref) / T));
    elseif(controller_idx == 3)
        addpoints(H.f5_vref1_3, t_now, ctrl.v_ref(1));
        addpoints(H.f5_vref2_3, t_now, ctrl.v_ref(2));
        addpoints(    H.f3_dist_3, t_now, min(vecnorm(p-o)) - 1);
        addpoints  (H.f3_tildev_3, t_now, norm(ctrl.v_ref   - ctrl.v));
        addpoints(H.f3_dinivref_3, t_now, norm((ctrl.v_ref-ctrl_last.v_ref) / T));
    end
end
