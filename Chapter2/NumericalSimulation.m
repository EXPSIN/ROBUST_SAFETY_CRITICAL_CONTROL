...
px = [  0, Time, Time,  0,   0];
py = [ -D,  -D,     D,  D,  -D];
patch(px,  py, 'c', 'facecolor', [1,1,1]*0.5, 'facealpha', 0.2, 'displayname', '非安全集合', 'handlevisibility', 'off', 'linestyle', 'none');
klinestyle = {'-', '--', ':', '-.'};
...
for i_Sim  = 1:length(k_h)
    ...
    for k = 1:N
        ...
        % collision avoidance
        v_c = -1;
        v_set = collision_avoidance(p, v_c, D_s(i_Sim), k_h(i_Sim), o_1);
