...
addpath('Auxiliary/');
load('E250809_174230.mat');     % k = 1.00
% load('E250809_174358.mat');     % k = 1.50
% load('E250809_174556.mat');     % k = 0.75
...
for i = 1:2
    if(i == 1)
        mask  = (run_state == 12); % 常规方法
        if(sum(mask) == 0)
            continue;
        end
    else
        mask  = (run_state == 14); % 提出的方法
        if(sum(mask) == 0)
            continue;
        end
    end
    ...
    H_obs2_hatch = hatchfill(H_obs2, 'single', 45, 10, [1, 1, 1]); % 填充斜线
    H_obs2.FaceColor = 'w';
    ...
    % end
    % 
    % plot(t,  v_set(1, :),  'k-', 'LineWidth', 2, 'DisplayName', 'v set' );
    % plot(t,    v_c(1, :),  'b:', 'LineWidth', 2, 'DisplayName', 'v ref' );
    % plot(t, v_real(1, :), 'r-.', 'LineWidth', 2, 'DisplayName', 'v real');
    % legend;
    % subplot(2, 1, 2); 
    % if(i == 1)
    %     hold on; box on; grid on;
    ...
    %     xlim([0, max(t)]);
    % end
    % plot(t,  v_set(2, :),  'k-', 'LineWidth', 2,  'DisplayName', 'v set' );
    % plot(t,    v_c(2, :),  'b:', 'LineWidth', 2,  'DisplayName', 'v ref' );
    % plot(t, v_real(2, :), 'r-.', 'LineWidth', 2,  'DisplayName', 'v real');
    % legend;
    ...
    % set(gca, 'FontSize', 16);
    % xlim([0, max(t)]);
    % plot(t,  v_set(3, :),  'k-', 'LineWidth', 2,  'DisplayName', 'v set' );
    % plot(t,    v_c(3, :),  'b:', 'LineWidth', 2,  'DisplayName', 'v ref' );
    % plot(t, v_real(3, :), 'r-.', 'LineWidth', 2,  'DisplayName', 'v real');
    % legend;
end
