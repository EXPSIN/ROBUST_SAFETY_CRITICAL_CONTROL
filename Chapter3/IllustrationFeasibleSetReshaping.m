...
% [norm_original, norm_composing, norm_composing2, norm_nonsmooth, norm_reshaped] = norm_of_solution_compare_mex(X, Y, p_o, 1.0, D, D_s, [1; 1], [1;2]);
...
% H = drawRectangles(hAxes, zoom_range);
% quiver3(zoom_range(1)+0.0, zoom_range(3)-0.05, 2, -1.4, -0.8, 2, 1.0, 'filled', 'LineWidth', 2, 'MaxHeadSize', 0.5, 'color', 'b');
% 
% axes(hNewAxes); grid off;
% axis(zoom_range); xticks([]); yticks([]);
% set(hNewAxes, 'Position', get(hAxes, 'Position') + [0.02, 0.05, -0.35, -0.5], 'fontsize',14, 'xtick', []);    % 设置新坐标轴的位置


% hNewAxes_coordinate = copyobj(hAxes, gcf); % 创建一个新的坐标轴
% delete(allchild(hNewAxes_coordinate));     % 删除新坐标轴的子对象
% set(hNewAxes_coordinate, 'color', 'none', 'xcolor', 'k', 'ycolor', 'k', 'LineWidth', 0.5, 'FontWeight', 'normal');
...
% H = drawRectangles(hAxes2, zoom_range);
% 
% 
% quiver3(zoom_range(1)+0.0, zoom_range(3)-0.05, 2, -1.4, -0.8, 2, 1.0, 'filled', 'LineWidth', 2, 'MaxHeadSize', 0.5, 'color', 'b');
% 
...
% axis(zoom_range); xticks([]); yticks([]);
% set(hNewAxes2, 'Position', get(hAxes2, 'Position') + [0.02, 0.05, -0.35, -0.5], 'fontsize',14, 'xtick', []);    % 设置新坐标轴的位置
...
% H = cone_constraint_2D([], A, b, c_delta, [0; 0], axis_range, 'c', ['$\mathcal{U}_c$, $A_c=[1,0;0,1]$' newline '$~~~~~\,b_c=[2;2]$, $c_c=[0.2;0.2]$']);
% H = cone_constraint_2D([], A, b, c_delta, [0; 0], axis_range, 'c', ['$\mathcal{U}_c$']);
H = half_plane([], A, b, [0; 0], axis_range, 'c', '$\mathcal{V}$');
% H.handle_space.FaceAlpha = 0.6-i*0.1;
% H.handle_space.FaceColor = [71,167,200]'/255;
...
for i = 1
    ...
    % H_polyhedron.handle_space.FaceColor = clr_v{i};
...
% H_basis = quiver(zeros(size(b_L,1),1), zeros(size(b_L,1),1), ...
%     A_L(:,1).*b_L, A_L(:,2).*b_L, 1.0, 'linewidth', 0.5, 'color', 'k', 'MaxHeadSize',1);
