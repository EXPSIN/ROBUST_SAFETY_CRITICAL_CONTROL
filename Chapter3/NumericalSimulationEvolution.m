...
t = 0:T:20;
N = length(t);
show_reshape_times = floor(linspace(1, N-1, 9));
...
controller_idx = 3;
X  = [p0; x0];
p_show_reshape  = zeros(2, length(show_reshape_times));
p_show_reshape_cnt = 0;
for i = 1:N
    [dX, ctrl] = closed_loop(X, t, controller_idx);
    H = graphic(H, t(i));
    X = X + dX*T;
    
    if(any(i==show_reshape_times))
        p_show_reshape_cnt = p_show_reshape_cnt + 1;
        p_show_reshape(:, p_show_reshape_cnt) = X(1:2,   1);
    end
...
function [v_ref, A, b, A_L, b_L, u_L] = QP_with_reshaped_feasible_set(p, o, D, D_o, D_s, vc)
% persistent A_L;
% if(isempty(A_L))
n_L  = 11;
...
X   = evalin("base", 'X');
T   = evalin("base", 'T');
show_reshape_times   = evalin("base", 'show_reshape_times');
...
idx = find(show_reshape_times==ceil(t_now/T));
if(~isempty(idx))
    % figure(100+idx); hold on;
    % set(gcf, 'Position', [10+30*idx,10+30*idx,400,400], 'color', 'w');
    figure(100); hold on; %  axis equal;
    set(gcf, 'Position', [10,10,1000,1000], 'color', 'w');
    subplot(3, 3, idx);
    ...
    v_c        = [5.0; 5.0]; 
    axis_range = [-1.0, 1.0, -1.0, 1.0]*1.2;
    [v_ref_original, A, b]              = QP_with_original_feasible_set(p, o, D, D_o, D_s, v_c);
    [v_ref_reshaped, ~, ~, A_L, b_L, ~] = QP_with_reshaped_feasible_set(p, o, D, D_o, D_s, v_c);
    H_original_feasible_set = half_plane([],   A,   b, [0; 0], axis_range, 'c', ' ' );
    H_reshaped_feasible_set = half_plane([], A_L, b_L, [0; 0], axis_range, 'g', ' ');

    % text(0.1, -0.8, sprintf('$t=%.1f$',        t_now),      'Interpreter', 'latex');
    % text(0.1, -1.0, sprintf('$p=[%.1f;%.1f]$', p(1), p(2)), 'Interpreter', 'latex');
    title(sprintf('$t=%.0f,p=[%.1f;%.1f]$', t_now, p(1), p(2)), 'Interpreter', 'latex');
    
    quiver(0, 0, v_ref_original(1), v_ref_original(2), 1.0, 'filled', 'LineWidth', 1.5, 'Color', [0.0, 0.0, 1.0], 'LineStyle', '-', 'MaxHeadSize', 5);
    quiver(0, 0, v_ref_reshaped(1), v_ref_reshaped(2), 1.0, 'filled', 'LineWidth', 1.5, 'Color', [0.0, 0.3, 0.0], 'LineStyle', ':', 'MaxHeadSize', 5);
    ...
    set(gca, 'fontsize', 16, 'XAxisLocation', 'origin', 'YAxisLocation', 'origin');
    % if(idx <= 5)
    %     set(gca, 'position', [0.01+(idx-1)*0.185, 0.53, 0.17, 0.40]);
    % else
    %     set(gca, 'position', [0.01+(idx-5)*0.185, 0.00, 0.17, 0.40]);
    % end
