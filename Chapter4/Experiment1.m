...
ylabel_handle.Units = 'normalized';  % 也可以设为 'data' 看实际效果
ylabel_handle.Position = [0.05, 0.9, 0];  % [x, y, z]，根据实际情况调整
ylim([-1, 1]);
xlim([0,5]);
yticks(-0.5:0.5:0.5);
...
if(isempty(H))
    ...
    H.fig3_legend = legend('Interpreter', 'latex');
    H.fig3_legend.AutoUpdate = "off";
    % H.fig3_vx_deadlock_resolving = animatedline('Color', 'b', 'LineStyle', '--');
    % H.fig3_vy_deadlock_resolving = animatedline('Color', 'k', 'LineStyle', '--');
    ...
    % addpoints(H.fig3_vx_original          , t, feedback.cmd.vx_local);
    % addpoints(H.fig3_vy_original          , t, feedback.cmd.vy_local);
    % addpoints(H.fig3_vx_deadlock_resolving, t, feedback.deadlock_resolving.vx_local);
    % addpoints(H.fig3_vy_deadlock_resolving, t, feedback.deadlock_resolving.vy_local);
...
function plotSLAMMap(slamAlg, handle_pointcloud, handle_poses)
    % plotSLAMMap: 在指定的 figure 句柄中绘制 SLAM 结果，包括机器人轨迹和激光扫描点云
    % 
    % 输入:
    %   slamAlg - lidarSLAM 对象，包含 SLAM 算法的结果
    %   figHandle - figure 句柄，用于指定显示的图窗
    %
    % 示例:
    %   plotSLAMMap(slamAlg, figure(1));
    ...
    % 汇总所有点云到一个大矩阵
    allScanPoints = [];  % 存储所有扫描点云的矩阵

    % 遍历每一个扫描，将扫描点云转移到对应位姿后汇总
    for i = 1:numel(scans)
        % 获取扫描的极坐标数据
