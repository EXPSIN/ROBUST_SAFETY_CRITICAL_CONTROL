...
figure(200);
res  = a_partial_spec(logical(eye(Nx)));
resO = aO_partial_spec(logical(eye(Nx)));
plot(1:Nx, res, 'r--*', 'Displayname', 'reshaple'); hold on;
plot(1:Nx, resO, 'b-*', 'Displayname', 'original');
legend;
...
clf; set(gcf, 'position', [800, 0, 650, 500 ], 'color', 'w');
...
%     H.shapeO(iO) = patch(p(1,iO)+Ds*sin(linspace(0, 2*pi, 30)), p(2,iO)+Ds*cos(linspace(0, 2*pi, 30)), 'c', 'facealpha', 0.2);
    H.realO(iO)  = patch(p(1,iO)+Do*sin(linspace(0, 2*pi, N)), p(2,iO)+Do*cos(linspace(0, 2*pi, N)), 10*ones(1, N),  'w', 'facealpha', 0.1, 'linewidth', 3);
end
...
text(p(1,2), p(2,2)-0.05, 'p_2', 'fontsize', 20, 'HorizontalAlignment', 'center');
text(p(1,3), p(2,3)-0.05, 'p_3', 'fontsize', 20, 'HorizontalAlignment', 'center');

% xLabelHandle = xlabel('$[p_1]_{1}$','interpreter','latex', 'FontSize', 18); 
% yLabelHandle = ylabel('$[p_1]_{2}$','interpreter','latex', 'FontSize', 18); 
xlabel('[p_1]_{1}', 'FontSize', 17);  ylabel('[p_1]_{2}', 'FontSize', 17); 
caxis manual; caxis([0 upper]); %colorbar;
c = colorbar;
c.Label.String = '|v^*_1|';
...
RecPos = [-0.2, -0.1, 0.3, 0.3];
boxcolor = 'r';
rectangle('Position', RecPos, 'EdgeColor', boxcolor, 'LineWidth',2);
line([RecPos(1),           0.13], [RecPos(2),           -0.60], 'linewidth', 2, 'color', boxcolor);
line([RecPos(1)+RecPos(3), 0.60], [RecPos(2)+RecPos(4), -0.13], 'linewidth', 2, 'color', boxcolor);
a_zoomed = copyobj(gca, gcf);
set(a_zoomed, 'position', [0.47,0.05,0.35,0.35], 'XLim', RecPos(1)+[0,RecPos(3)], 'YLim', RecPos(2)+[0,RecPos(4)]);
axes(a_zoomed);
xLabelHandle = xlabel('$[p_1]_{1}$','interpreter','latex', 'FontSize', 18); 
xLabelPos = get(xLabelHandle, 'position');
set(xLabelHandle, 'position', [xLabelPos(1), -0.05, xLabelPos(3)]);
box off
end
...
for k = 1:nao
    ...
    b(count, :) = -alpha_v*(1/dist-1/Ds) /delta_bar;
...
function H = myGraphic(H, p, x, Ds, Ap_i, ap_i, delta)
persistent v_set_last;
if(isempty(v_set_last))
    v_set_last = evalin('base', 'v_set');
end
nao = size(p, 2);
na  = size(x, 2);
no  = nao - na;
...
if(isempty(H))
    ...
    for iA = 1:na
        ...
        H.trajA(iA) = animatedline('color', 'r', 'HandleVisibility', HandleVisibility, 'linewidth', 1.5, 'linestyle', 'none', 'marker', '.'); 
    end
    H.A = line(p(1, 1:na), p(2, 1:na), 'Marker', 'o', 'linestyle', 'none', 'color','r');
    ...
    set(gca, 'fontsize', 16);
    H.feasibleRegionFull = half_space_2D([], zeros(nao-1, 2), zeros(nao-1, 1), 5*[-1, 1, -1, 1], 'r', 'domain', 'Reshaped');
    H.feasibleRegion     = half_space_2D([], Ap_i, ap_i, 5*[-1, 1, -1, 1], 'b', 'domain', 'Original');
    H.v_p_hdl   = quiver(0, 0, 0.0, 0.0, 1.0, 'linewidth', 2, 'color', 'r', 'MaxHeadSize',2);
    H.v_set_hdl = quiver(0, 0, 0.0, 0.0, 1.0, 'linewidth', 2, 'color', 'g', 'MaxHeadSize',2, 'linestyle', '-');
    H.v_now_hdl = quiver(0, 0, 0.0, 0.0, 1.0, 'linewidth', 2, 'color', 'b', 'MaxHeadSize',2);
    legend([H.v_p_hdl, H.v_set_hdl, H.v_now_hdl], 'v^p', 'v_{set}', 'v_{setO}');
    
    % feasible region
    figure(3); clf;
    hold on; 
    grid on;
    set(gcf, 'position', [800, 530, 800, 500 ], 'color', 'w');
    set(gca, 'fontsize', 16);
    H.V     = animatedline('color', 'b', 'HandleVisibility', HandleVisibility, 'linewidth', 2, 'linestyle', '-', 'marker', '.'); 
    H.v_set = animatedline('color', 'k', 'HandleVisibility', HandleVisibility, 'linewidth', 2, 'linestyle', '-', 'marker', 'none'); 
    H.v_setd= animatedline('color', 'r', 'HandleVisibility', HandleVisibility, 'linewidth', 2, 'linestyle', '--', 'marker', 'none'); 
    lgd_handle = legend([H.V,H.v_set,H.v_setd], '$\max\{log_{10}(V(p^{\delta})), 0\}$', '$\max\{log_{10}(|v^*|), 0\}$', '$\max\{log_{10}(|v^{*d}|), 0\}$', 'location', 'northeast');
    set(lgd_handle,'Interpreter','latex', 'FontSize', 18)
%     legend([H.tildev_m, H.d_vset], 'tildev_m', 'max\{log_{10}(v^{*d}), 0%\}');
    ...
    try
        adata = evalin('base', 'adata');
        half_space_2D(H.feasibleRegion,     adata.Ap_i, adata.ap_i);
        half_space_2D(H.feasibleRegionFull, adata.A,    adata.b);
    catch
