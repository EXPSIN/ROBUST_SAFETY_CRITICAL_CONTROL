...
for k = 1:cfg.cnt
    ...
    if(cfg.is_experiment == true)
        ...
        [~, y] = ode15s(@(t, x)close_loop_system(x, cfg), [0, cfg.step], x);
        x = y(end, :)';
...
[vec dis obs_number_o]=df_detect(cfg, x);

reshape_o_2=phi_plus_dis(cfg,vec,dis,obs_number_o);
...
[vstar,~,existflag]=quadprog(H,f,A,B,[],[],[],[],zeros(1,2),opt);
...
function reshape_o_2=phi_plus_dis(cfg,vec,dis,obs_number_o)
if obs_number_o~=0
    ...
    if obs_number_o==1
        reshape_o_1=min(cfg.df,reshape_o);
    else
        reshape_o_1_x=min(reshape_o(:,:));
        reshape_o_1=min(reshape_o_1_x,cfg.df);
    end
    reshape_o_2=reshape_o_1';
else
    reshape_o_2=cfg.df*ones(cfg.ctrl_basis_cnt,1);
end
end
...
function [vec dis n]=df_detect(cfg, x)

n=size(cfg.map_obs_theta( :,vecnorm( cfg.map_obs_theta-x(1:2,:) ) <= (cfg.df+cfg.ds) ),2);
if size(cfg.map_obs_theta( :,vecnorm( cfg.map_obs_theta-x(1:2,:) ) <= (cfg.df+cfg.ds) ),2)==0
    vec=[0,0];
    dis=0;
else
    P=cfg.map_obs_theta(:,vecnorm(cfg.map_obs_theta-x(1:2,:))<=(cfg.df+cfg.ds));
    W=vecnorm(cfg.map_obs_theta-x(1:2,:));
    dis= W(:,vecnorm(cfg.map_obs_theta-x(1:2,:))<=(cfg.df+cfg.ds))'-cfg.ds;
    K=(cfg.map_obs_theta-x(1:2,:))./vecnorm(cfg.map_obs_theta-x(1:2,:));
    vec=K(:,vecnorm(cfg.map_obs_theta-x(1:2,:))<=(cfg.df+cfg.ds))';
end
