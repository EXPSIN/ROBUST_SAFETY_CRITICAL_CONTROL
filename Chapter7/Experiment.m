...
addpath('Auxiliary/');
% 2024.03.25
% H = draw_graphic('experimental_data/E240325_221453.mat', '-', '$k_{\alpha} = 4.0$', 'r');
...
% figure(5)
% setAxesFull(gcf, 0.1, 0.01, 0.02, 0.02, 0.05)
...
if(isempty(H))
    ...
    % H.fig3d_obstacles_ctrl = obstacle_show_2D([], u.safety_ctrl.obs_cell, u.safety_ctrl.r+u.safety_ctrl.margin, [0.8, 0.8, 0.8], 0.0, 1.0);
    ...
    % ylabel('X');
    % zlabel('Y');
    % set(gcf, 'position', [100, 100, 800, 450], 'color', 'w');
    % set(gca, 'fontsize', 16);
    % 
    % H.figVTOL_vtol      = vtol_show(x_vtol, y_vtol, z_vtol, theta, pitch, yaw, []);
    % H.figVTOL_vtol_traj = animatedline('Color', 'k', 'DisplayName', 'Trajectory, k_{\gamma_2}=?', 'LineWidth', 2.0);
    % H.figVTOL_obstacles_ctrl = obstacle_show_2D([], ctrl.obs_cell, ctrl.r+ctrl.margin, 'c', -1.0, 1.0);
    % H.figVTOL_obstacles_real = obstacle_show_2D([], ctrl.obs_cell, ctrl.r, [0.8, 0.8, 0.8], 0.0, 1.0);
...
% title(sprintf('x%.2f, y%.2f, z%.2f, r%.1f, p%.1f, y%.1f  %.1f', x.p(1), x.p(2), x.p(3), rad2deg(x.angle(1)), rad2deg(x.angle(2)), rad2deg(x.angle(3)), u.az_ref));
drawnow limitrate;
end
...
% c1 = cos(roll); c2 = cos(pitch); c3 = cos(yaw);
% s1 = sin(roll); s2 = sin(pitch); s3 = sin(yaw);
